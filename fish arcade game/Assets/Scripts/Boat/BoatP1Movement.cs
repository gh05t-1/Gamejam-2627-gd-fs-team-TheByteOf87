...
public class BoatP1Movement : MonoBehaviour
{
    private bool canMove = false;
    public static UnityAction<bool> toggleMovementP1;
    ...
    void Start()
    {
        toggleMovementP1 += ToggleMv;
        toggleMovementP1.Invoke(false);
    ...
    void FixedUpdate()
    {
        if (canMove)
        {
            Vector3 mv = new Vector3(InputSystem.actions["MoveP1"].ReadValue<Vector2>().x, 0, InputSystem.actions["MoveP1"].ReadValue<Vector2>().y);
